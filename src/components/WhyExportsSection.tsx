...
const WhyExportsSection = () => {
  ...
  const {
    isVisible
  } = useScrollTrigger(sectionRef, 0.2);
  ...
  return <section id="why-exports-section" ref={sectionRef} className="py-20 bg-gradient-to-b from-white to-green-50">
        ...
        <div className={`transition-all duration-1000 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
          ...
          <div className="mb-16 sm:mb-24">
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 sm:gap-16 items-center">
              <div className="relative lg:order-first order-last">
                ...
                <img src="/lovable-uploads/c515d203-be93-46ac-9f21-8ef7def9a637.png" alt="Textile workers in a garment manufacturing facility" className="relative w-full h-48 sm:h-64 lg:h-80 object-cover rounded-2xl shadow-2xl border-2 sm:border-4 border-white" />
              </div>
              <div className="space-y-4 sm:space-y-6 order-first lg:order-last">
                ...
                <p className="text-base sm:text-lg text-gray-700 leading-relaxed">Global markets dwarf domestic demand–particularly in developing nations–enabling economies of scale, which fosters global competitiveness. Competing abroad drives firms to learn, innovate, and climb the productivity ladder. Workers benefit from the creation of more and more productive jobs. Export earnings bolster foreign currency reserves and build resilience against external shocks.</p>
              </div>
            </div>
          </div>
          ...
          <div className="text-center bg-gradient-to-r from-green-600 to-blue-600 rounded-2xl sm:rounded-3xl p-6 sm:p-12 shadow-2xl">
            <div className="max-w-4xl mx-auto">
              ...
              <Button onClick={handleExploreMap} size="lg" className="bg-white text-green-600 hover:bg-green-50 px-6 sm:px-10 py-3 sm:py-4 text-lg sm:text-xl font-bold shadow-xl hover:shadow-2xl transition-all duration-300 transform hover:scale-105">
                <Map className="mr-2 sm:mr-3" size={20} />
                Explore the Map
              </Button>
              <a href="https://www.growth-teams.org/" target="_blank" rel="noopener noreferrer" className="flex items-center justify-center mt-6 text-white/80 text-sm sm:text-base hover:text-white transition-colors duration-200 cursor-pointer">
